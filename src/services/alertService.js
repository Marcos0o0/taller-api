...
const Product = require('../models/Product'); // Cambiado de Part a Product
...
class AlertService {
  ...
  async verificarStockBajo() {
    try {
      // Adaptado: usar campos de Product (stock y minStock)
      const productosStockBajo = await Product.find({
        $expr: { $lte: ['$stock', '$minStock'] },
        stock: { $gt: 0 }, // Excluir agotados
        isActive: true,
        isDeleted: false,
      });
      ...
      for (const producto of productosStockBajo) {
        ...
        const existente = await Alert.findOne({
          producto: producto._id, // Cambiado de 'repuesto' a 'producto'
          tipo: 'stock_bajo',
          estado: 'activa',
        });
  ...
  async verificarMovimientosInusuales() {
    try {
      // Buscar productos con más de 10 movimientos en las últimas 24 horas
        ...
        if (movimientosRecientes.length >= 10) {
          ...
          if (!existente) {
            await Alert.create({
              ...
              mensaje: `El producto ${producto.barcode} ha tenido ${movimientosRecientes.length} movimientos en las últimas 24 horas.`,
              datos: {
                stockActual: producto.stock,
                cantidadMovimientos: movimientosRecientes.length,
                tiposMovimientos: this.contarTiposMovimientos(movimientosRecientes),
              },
            });
   ...
   */
  contarTiposMovimientos(movimientos) {
  ...
  async enviarNotificaciones(alerta) {
    try {
      await alerta.populate('producto');
  ...
  async emitirActualizacionAlertas() {
    if (io) {
      ...
      io.emit('alertas-actualizadas', stats);
    }
  }
  ...
  async autoResolverAlertasStockNormalizado() {
    try {
      ...
      for (const alerta of alertasStockBajo) {
        ...
        // Si el stock ya está por encima del mínimo, resolver automáticamente
